...
- Do not add, remove, upgrade, or swap dependencies unless explicitly asked.
- Do not modify package.json or lockfiles without approval.
- If a requested component/pattern is missing locally, stop and ask before substituting another library.
- When the user links docs for a specific library, use that library’s approach only.
